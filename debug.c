...
bool debugger_command_flag,  /* TRUE during debug command */
...
typedef struct {                        /* watch structure */
  ...
  bool         store_flag;         /* TRUE to trace stores */
  bool         fetch_flag;         /* TRUE to trace fetches */
...
void execute_debugger_command(void);
...
WATCH_STRUCT_PTR allocate_watch(void);
...
)
{
  ...
  */
  if ((idp->info.ptr != NULL) && ((WATCH_STRUCT_PTR) idp->info.ptr)->store_flag) {
...
)
{
  ...
  */
  if ((idp->info.ptr != NULL) && ((WATCH_STRUCT_PTR) idp->info.ptr)->fetch_flag) {
  	...
  	case SEMICOLON: {
	    ...
	    */
	    ...
	    for (i = 0; i < watch_count; ++i) {
    		idp = watch_list[i];
		    if (idp != NULL) {
		      wp = (WATCH_STRUCT_PTR) idp->info.ptr;
		      printf ("%16s  ", idp->name);
    ...
    case IDENTIFIER: {
      ...
      switch (idp->defn.key) {
        ...
        case VARPARM_DEFN:  {
          ...
          */
          if (idp->info.ptr != NULL)
            return((WATCH_STRUCT_PTR) idp->info.ptr);
          else if (watch_count < MAX_WATCHES) {
            wp = alloc_struct(WATCH_STRUCT);
            wp->store_flag = false;
            wp->fetch_flag = false;

            idp->info.ptr = (char *) wp;
