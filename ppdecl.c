...
bool const_flag, type_flag, var_flag;     /* TRUE if keywords already printed */
...
void print_type(TYPE_STRUCT_PTR tp, bool defn_flag);
void print_enum_type(TYPE_STRUCT_PTR tp);
void print_subrange_type(TYPE_STRUCT_PTR tp);
void print_subrange_limit(int limit, TYPE_STRUCT_PTR range_tp);
void print_array_type(TYPE_STRUCT_PTR tp);
void print_record_type(TYPE_STRUCT_PTR tp);
void analyze_var_decl(SYMTAB_NODE_PTR var_idp);
...
void analyze_routine_header(SYMTAB_NODE_PTR rtn_idp)
{
  ...
  */
  if ((parm_idp != NULL) && (! (bool) rtn_idp->info.ptr)) {
	  bool sublist_done;
    ...
    */
    do {
	    ...
	    if ((rtn_idp->defn.key != PROG_DEFN) && (common_key == VARPARM_DEFN)) emit("VAR ");
	      emit(parm_idp->name);
...
void declaration_keyword(
  bool *flag,     /* TRUE if keyword already printed */
  char *string    /* keyword */
)
{
  if (! *flag) {
...
void analyze_const_defn(SYMTAB_NODE_PTR const_idp) /* constant id */
{
...
void analyze_type_defn(SYMTAB_NODE_PTR type_idp) /* type id */
{
  if (error_count > 0)
    return;
...
void print_type(
  ...
  bool defn_flag      /* TRUE if named definition */
)
{
  /*
  --  Identifier type.
...
void print_enum_type(TYPE_STRUCT_PTR tp)  /* type */
{
...
void print_subrange_type(TYPE_STRUCT_PTR tp) /* type */
{
...
void print_array_type(TYPE_STRUCT_PTR tp)  /* type */
{
  int             save_left_margin;   /* current left margin */
...
{
  int             save_left_margin;      /* current left margin */
  bool            sublist_done;          /* TRUE iff done */
...
void analyze_var_decl(SYMTAB_NODE_PTR var_idp) /* variable id */
{
  TYPE_STRUCT_PTR common_tp;          /* type of id sublist */
  bool            sublist_done;       /* TRUE iff sublist done */
