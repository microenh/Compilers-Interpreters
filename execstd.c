...
void exec_read_readln(SYMTAB_NODE_PTR rtn_idp);
void exec_write_writeln(SYMTAB_NODE_PTR rtn_idp);
TYPE_STRUCT_PTR exec_eof_eoln(SYMTAB_NODE_PTR rtn_idp);
...
void exec_read_readln(SYMTAB_NODE_PTR rtn_idp)  /* routine id */
{
...
void exec_write_writeln(SYMTAB_NODE_PTR rtn_idp) /* routine id */
{
  TYPE_STRUCT_PTR parm_tp;            /* parm type */
  int             field_width;
  int             precision;
	  ...
	  do {
	    ...
	    */
	    if (parm_tp == integer_typep)
		    ...
		    printf("%*s", field_width, tos->integer == 1 ? "TRUE" : "FALSE");
	    else if (parm_tp == char_typep)
		    printf("%*c", field_width, tos->byte);
	    else if (parm_tp->form == ARRAY_FORM) {
		    char buffer[MAX_SOURCE_LINE_LENGTH];

    		strncpy(buffer, tos->address, parm_tp->info.array.elmt_count);
...
TYPE_STRUCT_PTR exec_eof_eoln(SYMTAB_NODE_PTR rtn_idp) /* routine id */
{
  char ch = getchar();
...
TYPE_STRUCT_PTR exec_abs_sqr(SYMTAB_NODE_PTR rtn_idp) /* routine id */
{
...
TYPE_STRUCT_PTR exec_arctan_cos_exp_ln_sin_sqrt(SYMTAB_NODE_PTR rtn_idp) /* routine id */
{
  TYPE_STRUCT_PTR parm_tp;            /* actual parameter type */
  ...
  if (((code == LN)   && (tos->real <= 0.0)) || ((code == SQRT) && (tos->real <  0.0)))
...
TYPE_STRUCT_PTR exec_pred_succ(SYMTAB_NODE_PTR rtn_idp) /* routine id */
{
  TYPE_STRUCT_PTR parm_tp;            /* actual parameter type */
