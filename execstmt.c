...
void exec_assignment_statement(SYMTAB_NODE_PTR idp) /* target variable id */
{
  ...
  */
  ...
  } else if ((target_tp->form == ARRAY_FORM) || (target_tp->form == RECORD_FORM)) {
	/*
	--  array  := array
	--  record := record
    */
    ...
    while (size--)
      *ptr1++ = *ptr2++;
  } else if ((base_target_tp == integer_typep) || (target_tp->form == ENUM_FORM)) {
      /*
...
TYPE_STRUCT_PTR exec_routine_call(SYMTAB_NODE_PTR rtn_idp)  /* routine id */
{
...
TYPE_STRUCT_PTR exec_declared_routine_call(SYMTAB_NODE_PTR rtn_idp) /* routine id */
{
...
void exec_actual_parms(SYMTAB_NODE_PTR rtn_idp) /* id of callee routine */
{
  	...
  	if (formal_idp->defn.key == VALPARM_DEFN) {
	    ...
	    */
	    if (formal_tp->form == SUBRANGE_FORM) {
		    ...
		    value = ((base_formal_tp == integer_typep) || (base_formal_tp->form == ENUM_FORM))
			    ? tos->integer
			    : tos->byte;
