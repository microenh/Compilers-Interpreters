...
void write_writeln(SYMTAB_NODE_PTR rtn_idp) /* routine id */
{
  TYPE_STRUCT_PTR actual_parm_tp;     /* actual parm type */
  ...
  if (token == LPAREN) {
  	do {
	    ...
	    */
        ...
        && ((actual_parm_tp->form != ARRAY_FORM) || (actual_parm_tp->info.array.elmt_typep != char_typep)))
...
TYPE_STRUCT_PTR eof_eoln(SYMTAB_NODE_PTR rtn_idp) /* routine id */
{
  if (token == LPAREN) {
  	error(WRONG_NUMBER_OF_PARMS);
