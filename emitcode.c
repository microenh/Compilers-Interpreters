...
void emit_program_epilogue(SYMTAB_NODE_PTR prog_idp) /* id of program */
{
  SYMTAB_NODE_PTR np;
  int             i, length;
  ...
  */
  for (np = string_literal_list; np != NULL; np = np->next) {
  	fprintf(code_file, "%s_%03d\tDB\t\"", STRING_LABEL_PREFIX, np->label_index);
...
void emit_load_value(SYMTAB_NODE_PTR var_idp, TYPE_STRUCT_PTR var_tp)
{
  ...
  bool varparm_flag = var_idp->defn.key == VARPARM_DEFN;
