...
TYPE_STRUCT_PTR exec_expression(void);
TYPE_STRUCT_PTR exec_simple_expression(void);    
TYPE_STRUCT_PTR	exec_term(void); 
TYPE_STRUCT_PTR exec_factor(void);
TYPE_STRUCT_PTR	exec_constant(SYMTAB_NODE_PTR idp);
TYPE_STRUCT_PTR exec_variable(SYMTAB_NODE_PTR idp, USE use);
TYPE_STRUCT_PTR	exec_subscripts(TYPE_STRUCT_PTR tp); 
TYPE_STRUCT_PTR exec_field(void);
void promote_operands_to_real(
  STACK_ITEM_PTR operandp1,
  TYPE_STRUCT_PTR tp1, 
  STACK_ITEM_PTR operandp2,     
  TYPE_STRUCT_PTR tp2); 
...
TYPE_STRUCT_PTR exec_expression(void)
{
  ...
  */
  if ((ctoken == EQUAL) || (ctoken == LT) || (ctoken == GT)
    ...
    */
    else if ((result_tp->form == ARRAY_FORM) && (result_tp->info.array.elmt_typep == char_typep)) {
      int cmp = strncmp(operandp1->address, operandp2->address, result_tp->info.array.elmt_count);
...
TYPE_STRUCT_PTR exec_simple_expression(void)
{
  ...
  */
  while ((ctoken == PLUS) || (ctoken == MINUS) || (ctoken == OR)) {
    ...
    */
    if (op == OR) {
      operandp1->integer = operandp1->integer || operandp2->integer;
      result_tp = boolean_typep;
    }
...
TYPE_STRUCT_PTR exec_term(void)
{
  ...
  --  separated by operators.
  */
  while ((ctoken == STAR) || (ctoken == SLASH) || (ctoken == DIV) || (ctoken == MOD)  || (ctoken == AND)) {
	  op = ctoken;                        /* remember operator */
	  result_tp = base_type(result_tp);
...
TYPE_STRUCT_PTR exec_constant(SYMTAB_NODE_PTR idp) /* constant id */
{
...
)
{
  ...
  */
  if ((idp->defn.key == VARPARM_DEFN) && (tp->form != ARRAY_FORM) && (tp->form != RECORD_FORM))
  ...
  */
  ...
  while ((ctoken == LBRACKET) || (ctoken == PERIOD)) {
    if (ctoken == LBRACKET)
      tp = exec_subscripts(tp);
    else if (ctoken == PERIOD)
      tp = exec_field();
  }
  ...
  */
  ...
  if ((use != TARGET_USE) && (use != VARPARM_USE))
    trace_data_fetch(idp, tp, (tp->form == ARRAY_FORM) || (tp->form == RECORD_FORM)
      ? tos->address
      : tos);
...
TYPE_STRUCT_PTR exec_subscripts(TYPE_STRUCT_PTR tp) /* ptr to type structure */
{
  int subscript_value;
  ...
  while (ctoken == LBRACKET) {
    ...
    */
    do {
	    ...
	    --  Range check.
	    */
	    if ((subscript_value < tp->info.array.min_index) || (subscript_value > tp->info.array.max_index))
		    runtime_error(VALUE_OUT_OF_RANGE);

	    /*
	    --  Modify the data area address.
	    */
	    tos->address += (subscript_value - tp->info.array.min_index) * tp->info.array.elmt_typep->size;

	    if (ctoken == COMMA)
        tp = tp->info.array.elmt_typep;
	  } while (ctoken == COMMA);
...
)
{
  if (tp1 == integer_typep)
    operandp1->real = operandp1->integer;
  if (tp2 == integer_typep)
    operandp2->real = operandp2->integer;
}
